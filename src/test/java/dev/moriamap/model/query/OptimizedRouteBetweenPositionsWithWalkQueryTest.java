...
class OptimizedRouteBetweenPositionsWithWalkQueryTest {
	...
	{
		try {
			tn = TransportNetworkParser.generateFrom(OptimizedRouteBetweenPositionsWithWalkQueryTest.class.getResourceAsStream("/test_map_data.csv"));
	...
	{
		try {
			brokenTN = TransportNetworkParser.generateFrom(OptimizedRouteBetweenPositionsWithWalkQueryTest.class.getResourceAsStream("/test_map_data.csv"));
		} catch (InconsistentCSVException e) {
			throw new RuntimeException(e);
		}
	}
