...
/**
 * A WalkSegment represents the action of walking from a geographic vertex to another geographic
 * vertex.
 */
public class WalkSegment extends Edge {
  ...
  /**
   * Class constructor specifying origin and destination. The ends are considered on Earth.
   *
  ...
  public WalkSegment(GeographicVertex from, GeographicVertex to) {
    super(from, to);
    this.distance = from.getGeographicPosition().distanceFrom(to.getGeographicPosition());
  }

  /** {@return the travel duration of this WalkSegment} */
  public Duration travelTime() {
    return Duration.ofSeconds((long) (this.distance / (WalkSegment.WALK_SPEED / 3.6)));
  }
}
