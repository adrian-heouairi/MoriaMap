...
public class GeographicVertex implements Vertex {
  ...
  public static GeographicVertex at(GeographicPosition pos) {
    if (pos == null) throw new IllegalArgumentException("Position can not be null");
  ...
  /**
   * Given a center and a list of GeographicVertices (which may include the center), returns a
   * sorted map associating each GeographicVertex to its distance on Earth in meters from the
   * center. The center is not in the sorted map.
   *
  ...
  public static SortedMap<Double, GeographicVertex> makeDistanceSortedMap(
      GeographicVertex center, List<GeographicVertex> geographicVertices) {
  ...
  /**
   * {@return a list of between 0 and n elements, containing the geographic vertices of
   * distanceSortedMap which are the closest to its center and inside the circle of radius radius}
   *
   * @param n the maximum number of GeographicVertices to return
   * @param radius the radius of the circle inside which returned elements must be, in meters
   * @param distanceSortedMap the geographic vertices we want to consider, in a centered sorted map.
   *     See {@link #makeDistanceSortedMap(GeographicVertex, List)}
   */
  public static List<GeographicVertex> getNClosestGVsWithinRadius(
      int n, double radius, SortedMap<Double, GeographicVertex> distanceSortedMap) {
    Set<Double> keys = distanceSortedMap.keySet();
    Object[] results =
        keys.stream().filter(d -> d <= radius).limit(Math.min(keys.size(), n)).toArray();
   ...
   * {@return a list of between 0 and n elements, containing the geographic vertices of
   * distanceSortedMap which are the closest to its center and inside the circle of radius radius.
   * If none are inside the circle, we return a list containing only one element, which is the
   * closest GeographicVertex to the center, no matter the distance}
   *
   * @param n the maximum number of GeographicVertices to return
   * @param radius the radius of the circle inside which returned elements must be, in meters
   * @param distanceSortedMap the geographic vertices we want to consider, in a centered sorted map.
  ...
  public String toString() {
    return String.format(
        "(%3.2f,%3.2f)",
        this.geographicPosition.getLatitude(), this.geographicPosition.getLongitude());
  }
}
