...
public class DFSTraversalStrategy implements TraversalStrategy {
   ...
   * stopAtDestination is true, following the Depth-First Search manner. As this strategy does not
   * involve weights, they are not used. The map returned corresponds to the association of each
   * visited Vertex to its incoming Edge during the traversal.
   *
  ...
  public Map<Vertex, Edge> traversal(
    ...
    if (stopAtDestination) Objects.requireNonNull(dst);
    Objects.requireNonNull(graph);
    if (!graph.contains(src)) throw new NoSuchElementException("Absent source Vertex");
    if (src.equals(dst) && stopAtDestination) return new HashMap<>();
    ...
    while (!stack.isEmpty()) {
      ...
      for (Edge outgoingEdge : graph.getOutgoingEdgesOf(tmp)) {
        ...
        if (!visited.contains(to)) {
          ...
          if (to.equals(dst)) return parents;
          stack.push(to);
        }
      }
    }
