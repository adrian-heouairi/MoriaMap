...
public class DijkstraTraversalStrategy implements TraversalStrategy {
  ...
  /**
   * Explores the vertices of the given Graph, computing the shortest path from src to dst if
   * singleDestination is true or to all other vertices if false. weightFunction is used to
   * determine which Edge to take during the traversal. The map returned corresponds to the shortest
   * path tree from src to all other nodes in the given Graph if singleDestination is false. If
   * singleDestination is true, the map returned corresponds to the shortest path from src to dst.
   *
   ...
   * @param singleDestination a flag that indicates whether to stop when destination Vertex is found
   * @param graph the graph to explore
   * @return the shortest path tree from src to dst if singleDestination is true, the shortest path
   *     tree from src to all other vertices otherwise
  ...
  public Map<Vertex, Edge> traversal(
    ...
    if (singleDestination) Objects.requireNonNull(dst);
    Objects.requireNonNull(weightFunction);
    Objects.requireNonNull(graph);

    List<Vertex> visited = new ArrayList<>();
    Map<Vertex, Double> distance = new HashMap<>();
    for (Vertex v : graph.getVertices()) distance.put(v, Double.POSITIVE_INFINITY);
    ...
    while (!notVisited.isEmpty()) {
      ...
      if (hasToStop(a, dst, singleDestination)) return res;
      ...
      for (Edge ab : graph.getOutgoingEdgesOf(a)) {
        if (!visited.contains(ab.getTo())) {
          ...
          double distanceFromSrcToB = distA + weightFunction.apply(distA, ab);
          Vertex to = ab.getTo();
