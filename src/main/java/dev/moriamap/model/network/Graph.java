...
public abstract class Graph {
  private static final String NULL_ARGUMENT_ERROR_MSG = "Argument can not be null";
  ...
  public List<Edge> getEdges() {
    ...
    for (Map.Entry<Vertex, List<Edge>> entry : this.vertexToOutgoingEdges.entrySet()) {
      if (entry.getValue() != null) edges.addAll(entry.getValue());
    }
    return edges;
  }

  /**
   * Adds the specified vertex to this Graph. If it is already present, does nothing.
   *
   * @param vertex the Vertex to add
   * @throws IllegalArgumentException if the given vertex is null
   */
  protected void addVertex(Vertex vertex) {
    if (vertex == null) throw new IllegalArgumentException(NULL_ARGUMENT_ERROR_MSG);
    this.vertexToOutgoingEdges.putIfAbsent(vertex, null);
  }

  /**
   * Adds the specified edge to this Graph. If edge's source or destination are not already in this
   * Graph, they are added too. If the specified Edge is already present, does nothing.
   *
   * @param edge the Edge to add
   * @throws IllegalArgumentException if the given edge is null
   */
  protected void addEdge(Edge edge) {
    if (edge == null) throw new IllegalArgumentException("Edge can not be null");
    Vertex from = edge.getFrom();
    Vertex to = edge.getTo();
    if (!this.vertexToOutgoingEdges.containsKey(from)) this.addVertex(from);
    if (!this.vertexToOutgoingEdges.containsKey(to)) this.addVertex(to);
    ...
    if (outgoingEdges == null) {
      ...
      if (!outgoingEdges.contains(edge)) outgoingEdges.add(edge);
    }
  }
   ...
   *
   * @param parents a map that associates each visited Vertex to the Edge that led to it during a
   *     Graph traversal
  ...
  public static List<Edge> getRouteFromTraversal(
      Map<Vertex, Edge> parents, Vertex src, Vertex dst) {
    if (parents == null || src == null || dst == null)
      throw new IllegalArgumentException(NULL_ARGUMENT_ERROR_MSG);
    if (!parents.containsKey(dst)) throw new NoSuchElementException("Destination is absent");
  ...
  private static boolean isVertexSourceOfEdgeInTraversalMap(
      Map<Vertex, Edge> parents, Vertex source) {
    Edge current;
    for (Map.Entry<Vertex, Edge> entries : parents.entrySet()) {
      current = entries.getValue();
      if (current != null && current.getFrom().equals(source)) return true;
    }
    return false;
  }
   ...
   * @throws NullPointerException if src is null or if dst is null and stopAtDestination is true
   */
  public Map<Vertex, Edge> traversal(
    ...
    return this.traversalStrategy.traversal(src, dst, weightFunction, stopAtDestination, this);
  }
  ...
  /**
   * Removes the given vertex from the graph and all edges that have this vertex on either side.
   *
  ...
  protected void removeVertex(Vertex v) {
    ...
    if (this.vertexToOutgoingEdges.containsKey(v)) {
      for (Edge e : this.getEdges()) {
        if (e.getTo().equals(v)) removeEdge(e);
      }
