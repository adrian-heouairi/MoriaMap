...
/**
 * A Passages object represents every passage/departure of every transport at a given stop for one
 * day. A Passages object containing nothing can exist, if no transports are circulating for a day.
 */
public final class Passages {
  ...
  /**
   * Returns a big unsorted string containing one line for each scheduled passage/departure for one
   * day at the stop this Passages object is for. Example of line: At La Fourche: line 13 direction
   * Châtillon Montrouge (variant 3): 19:57
   *
  ...
  private List<TransportSchedule> getTransportScheduleForTheGivenLineAndVariant(
      String lineName, String variantName) {
    List<TransportSchedule> res = new ArrayList<>();
    for (TransportSchedule ts : transportSchedules) {
      if (ts.variant().getName().equals(variantName)
          && ts.variant().getLineName().equals(lineName)) {
        res.add(ts);
      }
    }
    return res;
  ...
  /**
   * This method returns the time at which the next transport of (lineName, variantName) will come
   * at the Stop this Passages is for. It returns null if there is no time. If there are no
   * transports left for the day, the time of the first transport of the next day is returned.
   *
   ...
   * @return time at which next transport will come. If waitStart is equal to the next transport
   *     time, it returns waitStart
   */
  public LocalTime getNextTimeWithWrap(LocalTime waitStart, String variantName, String lineName) {
    ...
    List<TransportSchedule> tsByLineAndVariant =
        getTransportScheduleForTheGivenLineAndVariant(lineName, variantName);
  ...
  /**
   * This method returns the time one has to wait to ride the next transport of (lineName,
   * variantName) coming to the Stop this Passages is for. It returns a Duration of zero if there is
   * transport at waitStart. If there are no transports, it returns null. If the next transport
   * comes the next day (after midnight), that's okay.
   *
