...
public class PrettyPrinter {
   ...
   * @return duration formatted as a string format: H hour(s) M minute(s) S second(s)
   */
   ...
   * @return the localtime formatted as a string format: HH:MM:SS
   */
  public static String formatLocalTime(LocalTime lt) {
    Objects.requireNonNull(lt);
    return (lt.getHour() < 10 ? "0" : "")
        + lt.getHour()
        + ":"
        + (lt.getMinute() < 10 ? "0" : "")
        + lt.getMinute()
   ...
   * @return "null line" if the line was not found or a string build in the format: ## [Line and
   *     variant names] ## [Terminus of the variant] [the 'from' stop of the transport segment]
   * @throws IllegalArgumentException if line is null
   */
  public static String lineChangeToString(
      TransportNetwork tn, String lineName, TransportSegment segment) {
  ...
  public static String printTransportSegmentPath(TransportNetwork tn, List<Edge> path) {
    ...
    for (int i = 0; i < path.size(); i++) {
      ...
      else stringBuilder.append(segment.getTo());
    }
    return stringBuilder.toString();
  }

  private static Duration getEdgeDuration(Edge edge) {
    if (edge instanceof TransportSegment segment) return segment.getTravelDuration();
    else if (edge instanceof WalkSegment segment) return segment.travelTime();
    throw new UnsupportedOperationException("Segment type not yet supported");
  }

  private static String getWalkSegmentDescription(
      WalkSegment segment, String startMessage, LocalTime startAt) {
    return "\n\n"
        + startMessage
        + ": "
  ...
  private static String getTransportSegmentDescription(
    ...
    if (!segment.getLineName().equals(currentLine)) {
      res +=
          "\n\nArrival at: "
              + formatLocalTime(lts.get(i - 1).plus(getEdgeDuration(route.get(i - 1))))
              + ", departure at: "
              + formatLocalTime(lts.get(i))
              + lineChangeToString(tn, segment.getLineName(), segment);
    }
    res += " --> ";
    if (i == route.size() - 1) res += "\033[42m" + segment.getTo() + FORMAT_RESET;
    else res += segment.getTo();
    return res;
  }

  /**
   * This method build a string that contain all the information to travel on the transport network,
   * and with times at every line change
   *
   ...
   * @return "" if route is empty or the built String with all the information format: 'Switching
   *     line at [time]' (or for the first:'Taking line at: [starting time]') [when changing line]
   *     ## [Line and variant names] ## [terminus of the variant] [list of all the stops to go
   *     through on the same line: s1 -> s2 -> s3]
   */
  public static String printTransportSegmentPathWithLineChangeTimes(
      TransportNetwork tn, List<Edge> route, List<LocalTime> lts) {
    ...
    for (int i = 0; i < route.size(); i++) {
      ...
      if (edge instanceof TransportSegment segment) {
        ...
        if (i == route.size() - 1)
          builder
              .append("\n\nArrival time : ")
              .append(formatLocalTime(lts.get(i).plus(segment.getTravelDuration())));
      } else if (edge instanceof WalkSegment segment && i > 0) {
        builder.append(
            getWalkSegmentDescription(
                segment, "Arrival at", lts.get(i - 1).plus(getEdgeDuration(route.get(i - 1)))));
        if (i == route.size() - 1)
          builder
              .append("\n\nArrival time : ")
              .append(formatLocalTime(lts.get(i).plus(segment.travelTime())));
      }
    }
